...
export default function SignupPage() {
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-zinc-950 font-sans selection:bg-pink-500/30">
        ...
        <div className="mx-auto w-full max-w-md animate-in fade-in slide-in-from-bottom-8 duration-700">
          <div className="mb-10 text-center lg:text-left">
            <h1 className="text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">
              Rejoignez <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-orange-400">l'aventure</span>
            </h1>
            <p className="mt-3 text-base text-gray-500 dark:text-gray-400 font-medium">
              Créez votre compte {process.env.NEXT_PUBLIC_APP_NAME || "Linkaïa"} en quelques secondes.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5 group">
                <Label htmlFor="prenom" className={`text-sm font-semibold transition-colors ml-1 ${errors.prenom ? "text-red-500" : "text-gray-700 dark:text-gray-300 group-focus-within:text-pink-500"}`}>
                  Prénom
                </Label>
                <Input
                  id="prenom"
                  ...
                  placeholder="Jean"
                />
                {errors.prenom && <p className="text-xs text-red-500 ml-1 font-medium animate-in fade-in slide-in-from-top-1">{errors.prenom}</p>}
              </div>

              <div className="space-y-1.5 group">
                <Label htmlFor="nom" className={`text-sm font-semibold transition-colors ml-1 ${errors.nom ? "text-red-500" : "text-gray-700 dark:text-gray-300 group-focus-within:text-pink-500"}`}>
                  Nom
                </Label>
                <Input
                  id="nom"
                  ...
                  placeholder="Dupont"
                />
                {errors.nom && <p className="text-xs text-red-500 ml-1 font-medium animate-in fade-in slide-in-from-top-1">{errors.nom}</p>}
              </div>
            </div>
            ...
            <div className="space-y-1.5 group">
              <Label htmlFor="password" className={`text-sm font-semibold transition-colors ml-1 ${errors.password ? "text-red-500" : "text-gray-700 dark:text-gray-300 group-focus-within:text-pink-500"}`}>
                Mot de passe
              </Label>
              <Input
                id="password"
            ...
            <div className="space-y-3 rounded-2xl border border-pink-100 dark:border-pink-900/30 bg-pink-50/50 dark:bg-pink-900/10 p-5 shadow-sm">
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Votre mot de passe doit contenir :</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {[
                  ...
                  <div key={req.key} className="flex items-center gap-2 text-sm">
                    ...
                    <span className={passwordValidation.checks[req.key as keyof typeof passwordValidation.checks] ? "text-pink-600 dark:text-pink-400 font-medium" : "text-gray-500 dark:text-gray-400"}>
                      {req.label}
                    </span>
                  </div>
                ))}
          ...
          <p className="mt-8 text-center text-sm text-gray-500 dark:text-gray-400 font-medium">
            ...
            <Link href="/signin" className="font-bold text-pink-500 hover:text-pink-600 transition-colors hover:underline underline-offset-4">
              Connectez-vous ici
            </Link>
          </p>
        </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative items-center justify-center p-16 overflow-hidden bg-gradient-to-br from-pink-500 via-rose-400 to-orange-400">
        {/* Decorative background elements */}
        <div className="absolute inset-0 bg-black/5 mix-blend-overlay"></div>
        <div className="absolute -top-20 -right-20 w-96 h-96 bg-white/20 rounded-full blur-3xl animate-pulse" />
        <div className="absolute -bottom-20 -left-20 w-96 h-96 bg-purple-500/30 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />
